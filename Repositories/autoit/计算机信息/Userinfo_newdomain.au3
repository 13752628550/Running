...
$Sleeptime=18678789
$Scriptrun=""
$Scriptpah="\\172.26.32.251\softwa\pcsetup\domain\application"
$Setupfile="\\172.26.32.251\softwa\pcsetup\domain\inifile\parameter.ini"
...
_showrun()
while 1
	$vSleeptime=IniRead($Setupfile,"parameter","Sleeptime","")
	if $vSleeptime <> "" Then
		$sleeptime=$vSleeptime*1000*60
	EndIf
	$Scriptrun=IniRead($Setupfile,"parameter","Scriptrun","")
	if $Scriptrun <> "" Then
		filecopy($Scriptpah & "\" & $Scriptrun,@TempDir,1)
		RunWait(@TempDir & "\" & $Scriptrun)
	EndIf
	sleep($Sleeptime)
	_showrun()
wend
...
$IniFilebk=@TempDir & "\" & "Computer.ini"
FileCopy($IniFile,$IniFilebk,1)
;FileCopy($Inifile7,$Inifile8,$FC_OVERWRITE)
$InstallDate=""
if FileExists($IniFilebk) Then
	$vSerialNumber=IniRead($IniFilebk,$Computername,"SerialNumber","")
	if $vSerialNumber="" Then
		$InstallDate=@YEAR&"."&@MON&"."&@MDAY&" " &@HOUR&":"&@MIN&":"&@SEC
	EndIf
	if $vSerialNumber<>$SerialNumber Then
		$InstallDate=@YEAR&"."&@MON&"."&@MDAY&" " &@HOUR&":"&@MIN&":"&@SEC
	EndIf
Else
	$vSerialNumber=IniRead($IniFile,$Computername,"SerialNumber","")
	if $vSerialNumber="" Then
		$InstallDate=@YEAR&"."&@MON&"."&@MDAY&" " &@HOUR&":"&@MIN&":"&@SEC
	EndIf
	if $vSerialNumber<>$SerialNumber Then
	...
	EndIf
EndIf
$CHKDate=@YEAR&"."&@MON&"."&@MDAY&" " &@HOUR&":"&@MIN&":"&@SEC
...
if $InstallDate <> "" Then
	IniWrite($IniFile,$Computername,"InstallDate",$InstallDate)
EndIf
