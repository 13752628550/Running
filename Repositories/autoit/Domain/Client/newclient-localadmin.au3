...
While 1
	...
	Switch $nMsg
		...
		Case $Button1
			...
			GUICtrlSetData($label6,"已完成 1/4 ")
			Envionment()
			sleep(3000)
			GUICtrlSetData($Label5,"计算机设置中...")
			GUICtrlSetData($label6,"已完成 2/4 ")
			DomainJoi()
			sleep(3000)
			GUICtrlSetData($Label5,"用户数据设置...")
			GUICtrlSetData($label6,"已完成 3/4 ")
			Sleep(70000)
			Waituser()
			sleep(3000)
			GUICtrlSetData($Label5,"设置完成机器将在60秒后再启动！")
			GUICtrlSetData($label6,"已完成 4/4 ")
			sleep(3000)
			guisetstate(@SW_HIDE)
			ResetPC()
		Case $Button2
...
EndIf
;电脑添加新域控
RunWait(@ComSpec & " /c " & $DomainCommand2,"",@SW_HIDE )
If @OSArch="X86" Then
		$joinDomain= RegRead("HKLM\SYSTEM\ControlSet001\services\Tcpip\Parameters","Domain")
		$valueDomain=RegRead("HKLM\SYSTEM\CurrentControlSet\Control\Lsa\OfflineJoin","DNSDIDomName")
	Else
		$joinDomain= regread("HKLM64\SYSTEM\ControlSet001\services\Tcpip\Parameters","Domain")
		$valueDomain=RegRead("HKLM64\SYSTEM\CurrentControlSet\Control\Lsa\OfflineJoin","DNSDIDomName")
	EndIf
If $joinDomain="CTE.AD.LOCAL" or $valueDomain="CTE.AD.LOCAL" Then
	;msgbox ("","","设置完成系统将再启动")
	IniWrite($inifile,$ComputerName,"Date",@YEAR&"."&@MON&"."&@MDAY&" " &@HOUR&":"&@MIN&":"&@SEC )
	if $UserName <> "Migrate" Then
		IniWrite($inifile,$ComputerName,"Complete","2")
	Else
		IniWrite($inifile,$ComputerName,"Complete","3")
	EndIf
Else
...
Func Waituser()
	while 1
		...
		$vComplete3=IniRead($inifile,$ComputerName,"Complete","")
		if $vComplete3="3" then
			msgbox ("","","设置完成系统将再启动")
			ExitLoop
		EndIf
		sleep(1*1000*60)
